...
class HttpUtil:
    ...
    def __init__(self):
        self.SPEED_LIMIT_SIZE = 7 * 1024 * 1024
        self.SPEED_LIMIT_TIME = 3
        self.STOP = False
    def stop(self):
        self.STOP = True
    ...
    def append_file(self, _client, remote_path, local_path, remote_size=0, local_size=0, progress=None,
                    progress_args=()):
        if self.STOP:
            logger.info("append_file %s stop signal" % (remote_path))
            return
        ...
        with open(local_path, 'ab') as local_file:
            ...
            buff = 0
            speendLimit = self.SpeedLimit(0, _client.chunk_size, 0, 0)
            for block in response.iter_content(chunk_size=_client.chunk_size):
                ...
                # 限速
                self.speed_limit(_client.chunk_size, self.SPEED_LIMIT_SIZE, self.SPEED_LIMIT_TIME, speendLimit)
                if self.STOP:
                    logger.info("download file %s stop signal" % (remote_path))
                    return

    # 限速
    # 在speed_limit_time 这个单位时间内下载量不能超过 speed_limit_size 限速为 speed_limit_size/speed_limit_time(B/s)
    def speed_limit(self,increment, speed_limit_size, speed_limit_time, speed_limit):
        # 从开始下载开始累计下载量
